...
def makeInverseIndex(strlist):
    ...
    dct = {}
    LoW=[]
    for (i,doc) in enumerate(strlist): LoW = LoW + doc.split()
    for (i,word) in enumerate(LoW):
        if (word not in dct.keys()): dct[word] = []	
        for (j,sentence) in enumerate(strlist):
            sentence = sentence.split()
            for (k,words) in enumerate(sentence):
                if ((word in dct.keys()) and (word == words)):
                    dct[word].append(j)
    for key in dct.keys(): dct[key] = set(dct[key])
    return dct
...
def orSearch(inverseIndex, query):
    ...
    for (i,token) in enumerate(SearchString):
        SearchIndex = SearchIndex + list(inverseIndex[token])
    
    return (set(SearchIndex))
...
def andSearch(inverseIndex, query):
    ...
    for (i,token) in enumerate(SearchString):
        if i==0: 
            idx = list(inverseIndex[token])
        else: 
            idx = [val for val in idx if val in list(inverseIndex[token])]
    return (set(idx))
