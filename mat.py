...
def equal(A, B):
    ...
    # print (x.D[0]) # set of vectors
    # print (x.D[1]) # domain of vectors
    
    vecs_A={}
    vecs_B={}
    
    # create containers dictionaries that will eventually have 
    # row vector representation of Matrix 
    for i,v in enumerate(A.D[0]):
        vecs_A[v]=Vec(A.D[1],{})
    
    for i,v in enumerate(B.D[0]):
        vecs_B[v]=Vec(B.D[1],{})
    
    # convert Matrix A into a row of vectors
    for (k,v) in A.f.items():
        for (k2, v2) in vecs_A.items():
            if k[0]==k2:
                v2.f[k[1]] = v
    
    # convert Matrix B into a row of vectors
    for (k,v) in B.f.items():
        for (k2, v2) in vecs_B.items():
            if k[0]==k2:
                v2.f[k[1]] = v
                
    # first check for domain equality then
    # check of equality of vectors row by row
...
def getitem(M, k):
    ...
    for (ky,v) in M.f.items():
        if (ky[0]==k[0] and ky[1]==k[1]): 
            val = v
...
def setitem(M, k, val):
    ...
    x=k[0]
    y=k[1]
    if (x in M.D[0] and y in M.D[1]): M.f[(x,y)]=val
...
def add(A, B):
    ...
    True
    """
    final = {x: A.f.get(x, 0) + B.f.get(x, 0) for x in set(A.f).union(B.f)}
    return (Mat(A.D,final))
...
def scalar_mul(M, x):
    ...
    True
    """
    final = {i: M.f.get(i, 0)*x for i in set(M.f)}
    return (Mat(M.D,final))
...
def transpose(M):
    ...
    True
    """
    final = { (k[1],k[0]):v for k,v in M.f.items()}
    return Mat((M.D[1],M.D[0]),final)
...
def vector_matrix_mul(v, M):
    ...
    # create containers dictionaries that will eventually have 
    # row vector representation of Matrix 
    for i,v in enumerate(M.D[1]):
        vecs_M[v]=Vec(M.D[0],{})
        
    
    # convert Matrix A into a row of vectors
    for (k,v) in M.f.items():
        for (k2, v2) in vecs_M.items():
            if k[1]==k2:
                 v2.f[k[0]] = v
    ...
    for i in vecs_M:
        final[i] = sum([vec.f.get(x, 0) * vecs_M[i].f.get(x, 0) for x in set(vec.f).union(vecs_M[i].f)])
    
    return Vec(M.D[1],final)
...
def matrix_matrix_mul(A, B):
    ...
    for i,v in enumerate(B.D[1]):
        vecs_B[v]=Vec(B.D[0],{})
    
    # convert Matrix A into a dictionary of row of vectors
    for (k,v) in A.f.items():
        for (k2, v2) in vecs_A.items():
            if k[0]==k2:
                v2.f[k[1]] = v
    
    # convert Matrix B into a dictionary of column of vectors
    for (k,v) in B.f.items():
        for (k2, v2) in vecs_B.items():
            if k[1]==k2:
                v2.f[k[0]] = v
    
    # find the dot product between individual elemental vectors of A and B
    final = {}

    # calculate dot products
    for i in vecs_A:
        for j in vecs_B:
            dp = sum([vecs_A[i].f.get(x, 0) * vecs_B[j].f.get(x, 0) for x in set(vecs_A[i].f).union(set(vecs_B[j].f))])
            if dp !=0: final[(i,j)] = dp
    
    return (Mat((A.D[0],B.D[1]),final))
################################################################################
...
A = Mat(({0,1,2}, {'a','b'}), {(0,'a'):4, (0,'b'):-3, (1,'a'):1, (2,'a'):1, (2,'b'):-2})
B = Mat(({'a','b'}, {'x','y'}), {('a','x'):3, ('a','y'):-2, ('b','x'):4, ('b','y'):-1})
