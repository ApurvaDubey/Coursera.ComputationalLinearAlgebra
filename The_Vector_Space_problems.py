...
def vec_select(veclist, k):
    ...
    return ([Vec(x.D,x.f) for x in veclist if ((k in x.f and x.f[k]==0) or (k not in x.f))])
...
def vec_sum(veclist, D):
    ...
    if (len(veclist))==0:
        ...
        v_final=Vec(D,{})
        for d in D: v_final.f[d]=0

        for d in D:
            for x in veclist:
            #print (d,getitem(x,d))
                v_final.f[d]+=x.f.get(d,0)
            #print (x)

        
    mydict = { k : v for k,v in v_final.f.items() if v != 0 }
...
def vec_select_sum(veclist, k, D):
    ...
    True
    '''
    new_veclist = vec_select(veclist,k)

    return vec_sum(new_veclist,D)
