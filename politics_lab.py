...
def create_voting_dict(strlist):
    ...
    """
    return {x.split()[0]:list(map(int,x.split()[3:])) for (i,x) in enumerate(strlist)}
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    """
    return sum([score1*score2 for (i, score1) in enumerate(voting_dict[sen_a]) for (j,score2) in enumerate(voting_dict[sen_b]) if i == j])
...
def most_similar(sen, voting_dict):
    ...
    for x in voting_dict:
        if x != sen: 
            ...
            score = policy_compare(sen,x,voting_dict)
            if score > max:
                max = score
                best_match=x
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    """
    score=0
    for sen_i in sen_set:
        score=score+policy_compare(sen, sen_i, voting_dict)

    return (score/len(voting_dict))


##############
strlist=mylist
voting_dict={x.split()[0]:list(map(int,x.split()[3:])) for (i,x) in enumerate(strlist) if x.split()[1] == 'D'}
...
for sen in sen_set:
    #if sen=='Lieberman':
    #sen_set -=set(sen)
    avg_sim=find_average_similarity(sen,sen_set,voting_dict)
    print (sen,avg_sim)
        #print (voting_dict[sen])
    if avg_sim > maxnum:
        maxnum=avg_sim
        most_avg_sen = sen
        #print (sen,maxnum)

print (most_avg_sen,maxnum)
##############
