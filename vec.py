...
def equal(u,v):
    ...
    UnionSet=set(u.D).union(v.D)
    InterSet=set(u.D).intersection(v.D)

    
    #domain comparison and key and values comparison
    if (UnionSet == InterSet) and ({x: A.get(x, 0) for x in set(UnionSet)} == {x: B.get(x, 0) for x in set(UnionSet)}):
        flag = True
    else:
        flag = False
...
def add(u,v):
    ...
    final = {x: A.get(x, 0) + B.get(x, 0) for x in set(A).union(B)}

    return Vec(u.D,final)
...
def dot(u,v):
    ...
    dotprod = sum([A.get(x, 0) * B.get(x, 0) for x in set(A).union(B)])
...
def scalar_mul(v, alpha):
    ...
    final = {x: alpha * A.get(x, 0) for x in set(A)}

    return Vec(v.D,final)
