...
divisible_by_3 = not bool((673+909)%3)
...
LofL_sum = sum([x for x in sum(LofL,[])])
...
first_of_tuples_list = [(x,y,z) for x in S for y in S for z in S if x+y+z==0 if (x,y,z) != (0,0,0)][0]
...
list_sum_zip = list(x+y for x,y in zip(A,B))
...
representation_dict = { ((base**2)*x+(base**1)*y+(base**0)*z):(x,y,z) for x in digits for y in digits for z in digits}
